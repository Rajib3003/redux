...
import { selectTasks, setTasks, updateFilter } from "@/redux/features/task/taskSlice";
...
export default function Tasks() {
  const {data, isLoading} = useGetTaskQuery("undefinded",{
    refetchOnFocus: true,
  ...
  return (
    ...
    <div className="flex items-center justify-between mb-5 p-4 bg-white rounded-lg shadow-md mt-4 bg-white dark:bg-gray-800">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">         
        {!isLoading && tasks.map((task:ITask) => (<TaskCard task={task} key={task._id} />))}  
      </div>
